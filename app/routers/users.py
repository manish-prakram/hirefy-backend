...
@router.get('/techskills/')
async def get_user_technical_skills(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),):
    ...
    query_techSkills = db.query(models.TechnicalSkills).filter(
        models.TechnicalSkills.userProfileId == query_user.id).all()
